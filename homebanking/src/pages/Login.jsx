...
const DEMOS = [
  // Solo clientes — el personal del banco entra por el Core (:5173)
  { label:'cli000007', email:'cli000007@cliente.cmac.pe', hint:'demo1234' },
  { label:'cli000001', email:'cli000001@cliente.cmac.pe', hint:'demo1234' },
];
...
export default function Login() {
  ...
  const onRol = (rol) => {
    // Banco real: al portal del cliente SOLO entran clientes.
    if (rol && rol !== 'cliente') {
      fallar('Eres personal del banco. Ingresa al Core en http://localhost:5173');
      return;
    }
    ...
    <div style={{ minHeight:'100vh', display:'flex', fontFamily:"'DM Sans',system-ui,sans-serif", background:'#eef2f7' }}>
      ...
      <div style={{ flex:'1 1 54%', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
        <div style={{ width:'100%', maxWidth:420, background:'#fff', borderRadius:18, padding:'34px 34px 28px', boxShadow:'0 10px 40px rgba(6,42,82,.12)' }}>
          ...
          <div style={{ fontSize:11.5, color:'#0FA0AD', fontWeight:700, marginBottom:2 }}>PORTAL DEL CLIENTE · El personal del banco usa el Core (puerto 5173)</div>
          ...
          {modo === 'usuario' && (
            <form onSubmit={submitUsuario}>
              <Field label="Usuario (código de cliente o tarjeta)">
                <input value={usuario} onChange={e => setUsuario(e.target.value)} placeholder="cli000007" disabled={bloqueado} style={inp} />
              </Field>
              ...
              <div style={{ marginTop:16, background:'#f4f9fb', border:'1px solid #dbeafe', borderRadius:10, padding:'12px 14px', fontSize:12 }}>
                ...
                <div style={{ color:'#5b6b86', marginBottom:6 }}>Clientes de prueba · contraseña = demo1234</div>
                <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                  {CLIENTES.map(c => (
                    <button type="button" key={c} onClick={() => { setUsuario(c); setClave('demo1234'); }} style={chip}>{c}</button>
                  ))}
                </div>
              </div>
            </form>
